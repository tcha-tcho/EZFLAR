package {
	...
	public function addMarker (marker:FLARMarker) :void {
		...
		var objectsToAdd:Array = _objectsToAdd;
	    ...
	    while(i < len) {
			if(marker.patternId == objectsToAdd[i][0]){
	...
	public function removeMarker (marker:FLARMarker) :void {
	    ...
	    while(i < len) {
		trace("indice no markers: " + i + " ::: simbolo: " + this._markersByPatternId[i][0] + " childrens in Scene: " + this._scene3D.numChildren);
	...
	private function removePattern(toDelete:Array){
		var i:int = 0;
		var len:int = toDelete.length;
		trace(toDelete);
	    while(i < len) {
			this._scene3D.removeChild(this._markersByPatternId[toDelete[i]][2]);
			this._markersByPatternId.splice(toDelete[i],1);
	        i++;
	    }
	};
