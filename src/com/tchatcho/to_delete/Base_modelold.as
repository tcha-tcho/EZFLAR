package {
	...
	//
	//FOR ANIMATED DAEs - TODO: revisar esse codigo!
	...
	public class Base_model extends BasicView { //ou Sprite
		//to use models animated DAEs
		...
		public function addMarker (marker:FLARMarker) :void {
			...
			var materialsList:MaterialsList = new MaterialsList({all: this.transformById(marker.patternId)});
		...
		private function place_DAEs(patternId:int):DisplayObject3D{
			trace("testando erros");
/*			var DAEId:int = patternId % 5;
			switch (DAEId) {
				case 0:////////////////
				var container : DisplayObject3D = new DisplayObject3D();
				this._mCollada2 = new DAE(true, null, true);//loop in the constructor
				this._mCollada2.load("../resources/model/Scout/modelo.DAE");
				this._mCollada2.rotationZ = 270;
				this._mCollada2.scale = 1;
				_universe = new DisplayObject3D();
				_universe.addChild(this._mCollada2);
				_universe.z = 50;
				container.addChild( this._universe );
				this.scene3D.addChild(container);
				this._mCollada2..currentFrameNumber = 1;
				this._mCollada2.play();
				return container;
				
				case 1:///////////////////////////
				var container : DisplayObject3D = new DisplayObject3D();
				this._mCollada = new DAE();
				this._mCollada.load("../resources/model/eiffel/models/eiffel.dae");
				this._mCollada.rotationZ = 270;
				this._mCollada.scale = 1;
				_universe = new DisplayObject3D();
