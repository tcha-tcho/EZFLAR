...
public class LabelingBufferdImage extends BufferedImage
{
	...
	public void drawImage(FLARGlayscaleRaster i_raster) throws FLARException
	{
		assert (i_raster.getBufferReader().getBufferType() == IFLARBufferReader.BUFFERFORMAT_INT2D_GLAY_8);

		int w = this.getWidth();
		int h = this.getHeight();
		// サイズをチェック
		FLARIntSize size = i_raster.getSize();
	...
	public void drawImage(FLARBinRaster i_raster) throws FLARException
	{
		assert (i_raster.getBufferReader().getBufferType() == IFLARBufferReader.BUFFERFORMAT_INT2D_BIN_8);
	...
	public void overlayData(FLARIntPointStack i_stack)
	{
		int count = i_stack.getLength();
		FLARIntPoint[] items = (FLARIntPoint[])i_stack.getArray();
	...
	public void overlayData(FLARIntRectStack i_stack)
	{
		Color[] c=new Color[]{Color.cyan,Color.red,Color.green};
		int count = i_stack.getLength();
		FLARIntRect[] items = (FLARIntRect[])i_stack.getArray();
		Graphics g = this.getGraphics();
		for (int i = 0; i < count; i++) {
			int x = items[i].x;
			int y = items[i].y;
