package {
	...
	public function addMarker (marker:FLARMarker) :void {
		...
		for each (var object:Array in _objectsToAdd){
			if(marker.patternId == object[0]){
				var markerCopy:FLARMarker = new FLARMarker(marker.patternId, marker.direction, marker.confidence, marker.outline, marker.transformMatrix);
				this.containersByMarker[markerCopy] = placeModels(object[0], object[1], object[2]);
				markerList.push(markerCopy);
			}
		}
	}
	...
	public function removeMarker (marker:FLARMarker) :void {
		...
		private function init (numPatterns:uint) :void {
			var newNumPatterns = numPatterns + _objectsToAdd.length;
			this.markersByPatternId = new Array(newNumPatterns);
			while (newNumPatterns--) {
				this.markersByPatternId[newNumPatterns] = new Array();
			}
